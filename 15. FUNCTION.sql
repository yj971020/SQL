...
1. 문자열 함수
   ...
   ■ SUBSTR('문자열', 시작위치, 길이)
    ...
    > 모든 아이돌멤버의 나이를 출력하시오
        ----------------------------------------------------
        SELECT *,
            YEAR(current_date()) - substr(`BIRTHDAY`, 1, 4) as `Age`
        FROM 
        lecture.idol_member;
   ...
   ■ CONCAT()    문자열 이어주기
     CONCAT_WS() 구분자와 함께 문자열 이어주기
        ...
        SELECT CONCAT_WS('-','2021','01','20'); 구분자연결(WithSeparator)
        ----------------------------------------------------

    > 모든 아이돌그룹에 대하여 'BTS♥boys'의 형식과 같이 출력하시오
        ----------------------------------------------------
        SELECT CONCAT(`GROUP_NAME`, '♥', `GENDER`, 's') MSG
        FROM lecture.idol_group; 
        ----------------------------------------------------
   ...
   ■ REPLACE('문자열', '찾는 문자열', '대치할 문자열')
    ...
    > 아이돌 그룹에서 회사명 '~ 엔터테인먼트'를 '~ ENTERTAINMENT'로
      출력하시오
       ----------------------------------------------------
        SELECT 
            replace(COMPANY, '엔터테인먼트', 'ENTERTAINMENT') COMPANY,
            IG.GROUP_NAME, IG.DEBUT_ALBUM
        FROM lecture.idol_group IG;
       ----------------------------------------------------
...
3. 날짜함수
    ...
    > 아이돌멤버 중에서 생일이 지나지 않은 멤버를 모두 출력하시오
        ----------------------------------------------------
         ...
         SELECT `GROUP_NAME`, `MEMBER_NAME`, `BIRTHDAY`
         FROM idol_member
         WHERE MONTH(SYSDATE()) <= MONTH(`BIRTHDAY`); AND
                 DAY(SYSDATE()) <= DAY(`BIRTHDAY`);

         SELECT CONCAT(COUNT(`MEMBER_NAME`),'명') AS TOTAL 
         FROM idol_member
         WHERE MONTH(SYSDATE()) <= MONTH(`BIRTHDAY`) AND  
                 DAY(SYSDATE()) <= DAY(`BIRTHDAY`);
...
5. NULL 관련 함수
   : 컬럼 값이 NULL 인지 아닌지 확인 필요
    ...
    > 멤버테이블에서 전화번호(PHONE)가 NULL 이면 '9999-99-99'로 출력하시오 
        ----------------------------------------------------
         SELECT ID, PWD, NAME, '9999-99-99' BIRTHDAY
         FROM MEMBER
         WHERE BIRTHDAY IS NULL
         LIMIT 10, 100;
         
         SELECT ID, NAME, 
	            IFNULL(BIRTHDAY, '9999-99-99') AS `CHECK`
         FROM member
         LIMIT 10, 100;
        ----------------------------------------------------
